      ...
      <div className="flex h-14 items-center justify-between">
        <div className="flex items-center gap-6">
          <Link href="/" className="flex items-center gap-2">
            <div className="flex size-7 items-center justify-center rounded bg-[#10b981] text-sm font-bold text-[#0a0a0a]">
              М
            </div>
          ...
          <nav className="hidden items-center gap-4 lg:flex">
            {content.primaryNavigation.map((item, index) => (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex items-center gap-1 text-sm tracking-[-0.01em] transition-colors",
                  index === 0 ? "font-medium text-[#e5e5e5]" : "text-[#888] hover:text-[#e5e5e5]"
                )}
              >
                <span>{item.label}</span>
                {item.count ? (
                  <Badge className="h-4 rounded-[10px] bg-[#ef4444] px-[7px] text-[10px] font-medium text-white hover:bg-[#ef4444]">
                    {item.count}
                  </Badge>
                ) : null}
              </Link>
        ...
        <div className="flex items-center gap-3">
          <label className="relative hidden lg:block">
            <HugeiconsIcon
              icon={Search01Icon}
              size={16}
              className="pointer-events-none absolute top-1/2 left-3 -translate-y-1/2 text-[#666]"
            />
            <Input
              type="search"
          ...
          <Button
            variant="ghost"
            size="icon"
            className="size-9 rounded-[10px] text-[#e5e5e5] hover:bg-white/5 hover:text-white"
